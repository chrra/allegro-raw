...
{#fun unsafe al_get_next_event as getNextEvent' { id `EventQueue', alloca- `EventPtr' id } -> `Bool' #}
getNextEvent :: EventQueue -> IO (Maybe Event)
getNextEvent = ptrAndBoolToMaybe . getNextEvent'
{#fun unsafe al_peek_next_event as peekNextEvent' { id `EventQueue', alloca- `EventPtr' id } -> `Bool' #}
...
{#fun unsafe al_wait_for_event_until as waitForEventUntil' { id `EventQueue', alloca- `EventPtr' id, id `Timeout' } -> `Bool' #}
