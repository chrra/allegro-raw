...
AL_FUNC(int, al_join_paths, (ALLEGRO_PATH *path, const ALLEGRO_PATH *tail));
AL_FUNC(int, al_rebase_path, (const ALLEGRO_PATH *head, ALLEGRO_PATH *tail));
...
AL_FUNC(int, al_set_path_extension, (ALLEGRO_PATH *path, char const *extension));
AL_FUNC(const char*, al_get_path_basename, (const ALLEGRO_PATH *path));

AL_FUNC(int, al_make_path_canonical, (ALLEGRO_PATH *path));
