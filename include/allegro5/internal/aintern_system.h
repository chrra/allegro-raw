...
struct ALLEGRO_SYSTEM_INTERFACE
{
   ...
   int (*get_monitor_info)(int adapter, ALLEGRO_MONITOR_INFO *info);
   ...
   int (*get_cursor_position)(int *ret_x, int *ret_y);
   int (*grab_mouse)(ALLEGRO_DISPLAY *display);
   int (*ungrab_mouse)(void);
   ALLEGRO_PATH *(*get_path)(int id);
   int (*inhibit_screensaver)(int inhibit);
