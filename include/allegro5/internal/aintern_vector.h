...
AL_INLINE(int, _al_vector_is_empty, (const _AL_VECTOR *vec),
{
   ASSERT(vec);
   return vec->_size == 0 ? true : false; 
})
AL_INLINE(int, _al_vector_is_nonempty, (const _AL_VECTOR *vec),
{
   ASSERT(vec);
...
AL_FUNC(int, _al_vector_contains, (const _AL_VECTOR*, const void *ptr_item));
AL_FUNC(void, _al_vector_delete_at, (_AL_VECTOR*, unsigned int index));
AL_FUNC(int, _al_vector_find_and_delete, (_AL_VECTOR*, const void *ptr_item));
